...
class PatientListView(ListView):
    model = Patient
    #default: patient_list.html, context: patient_list

class PatientCreateView(CreateView):
    model = Patient
    success_url = reverse_lazy('index')
    fields = ('firstName','lastName', 'age')
    #default: patient_form.html, context: patient
...
class PatientDeleteView(DeleteView):
    model = Patient
    success_url = reverse_lazy('index')
    #default: patient_confirm_delete
    ...
    for eachEntry in data:
        if eachEntry.componentName == 'hw':
            ...
            if len(heightAndWeight) > 1:
                ...
                BMI = (float(heightAndWeight[1]))/(feetToMetres*feetToMetres)   #MBI = Weight/(height*height)
